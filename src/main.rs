...
impl FheSchnorr {
    ...
    fn sign(&self, message: &str, client_key: &ClientKey) -> Result<(u32, FheUint32), Box<dyn std::error::Error>> {
    ...
    fn verify(&self, message: &str, signature: (u32, FheUint32), client_key: &ClientKey) -> Result<FheBool, Box<dyn std::error::Error>> {
        // verify: s * g = r + e * pk
        ...
        let r_encrypted = FheUint32::try_encrypt(r, client_key)?;
...
mod tests {
    ...
    fn test_fhe_schnorr() {
        ...
        let signature = fhe_schnorr.sign(message, &client_key).unwrap();
        let end = Instant::now();
        println!("time taken for sign: {:?}", end.duration_since(start));

        let start = Instant::now();
        let result = fhe_schnorr.verify(message, signature, &client_key).unwrap();
        let end = Instant::now();
